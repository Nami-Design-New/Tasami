...
describe("useGetMeetings", () => {
  ...
  it("sends the current search term when fetching meetings", async () => {
    renderHook(() => useGetMeetings(), { wrapper: createWrapper() });

    await waitFor(() => expect(axiosInstance.get).toHaveBeenCalledOnce());

    expect(axiosInstance.get).toHaveBeenCalledWith("meeting", {
      params: {
        page: 1,
        community_id: "42",
        search: "planning",
