...
const FileUtils = {
  ...
  /**
   * Get a short label from a MIME type for display (e.g. "PDF", "DOCX")
   */
...
export default function FileUploader({
  ...
  /**
   * Render preview based on file type.
   * Images and videos render as before; all other file types show the
   * docs icon with a small type-badge so the user knows what was uploaded.
   */
  const renderPreview = useCallback(
    (preview, normalizedFile, index) => {
      console.log(preview, normalizedFile, index);
      ...
      return (
        <div
          className="uploaded_file_doc"
          data-testid={`file-doc-${index}`}
        ...
        >
          ...
          >
            {FileUtils.getFileTypeLabel(normalizedFile.mimeType)}
          </span>
        </div>
      );
    },
