...
const getCompletionCongratsKey = (workId) =>
  `goal_completion_congrats_seen_${workId}`;
...
export default function WorksTasks() {
  ...
  const handleAddTaskWithAi = (workId) => {
    addTaskWithAi(workId, {
      onSuccess: (res) => {
  ...
  useEffect(() => {
    if (goalTasks?.data) {
      const formatted = goalTasks.data.map((task) => ({
        ...task,
        id: String(task.id),
      }));
  ...
  // Show completion congrats modal one time only per work/goal
  useEffect(() => {
    const workId = workDetails?.id || id;

    if (!isCompletionReady || !workId) {
      setShowCompletionCongrats(false);
      return;
    }

    const storageKey = getCompletionCongratsKey(workId);
  ...
  }, [isCompletionReady, workDetails?.id, id]);
  ...
  const handleDragEnd = (event) => {
    ...
    setTasks((prev) => {
      const oldIndex = prev.findIndex((task) => task.id === String(active.id));
      const newIndex = prev.findIndex((task) => task.id === String(over.id));
