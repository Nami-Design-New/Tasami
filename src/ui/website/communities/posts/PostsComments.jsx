...
export default function PostsComments({ isSubscribed, isMyCommunity }) {
  ...
  const handleDeleteComment = (commentId) => {
    ...
    //  Optimistic comment → delete locally only
  ...
  const onSubmit = async (data) => {
    ...
    addPostComment(data.commentText, {
      onSuccess: () => {
        reset();
        // Refetch to replace temp comment with real one
        queryClient.invalidateQueries({
          queryKey: ["post-comments", `${id}`],
        });
      },
      onError: (err) => {
        ...
        queryClient.setQueryData(["post-comments", `${id}`], previousComments);
      },
    });
  };
          ...
          {allPostComments.map((comment) => (
            <div className="w-full mt-3" key={comment.id}>
              <ConsultationCommentsCard
                comment={comment}
                onDelete={handleDeleteComment}
                isDeleting={isDeleting || comment.isOptimistic}
              />
            </div>
          ))}
        </InfiniteScroll>
